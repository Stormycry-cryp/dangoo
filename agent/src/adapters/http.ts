...
export class HttpDangooGateway implements CanvasGateway, AssetGateway {
  private constructor(private base:URL,private tokenFor:(scope:Scope)=>Promise<string>,private caps:BridgeCapabilities,private fetcher:typeof fetch,private authHeader:'Authorization'|'X-Pb-Auth'){}
  static async connect(options:{baseUrl:string;tokenFor:(scope:Scope)=>Promise<string>;scope:Scope;fetch?:typeof fetch;authHeader?:'Authorization'|'X-Pb-Auth'}){
    ...
    const instance=new HttpDangooGateway(base,options.tokenFor,{contractVersion:CONTRACT_VERSION,revision:'pending',nodes:[],operations:[],jobs:false,assets:false},options.fetch??fetch,options.authHeader??'Authorization');
  ...
  private async request<T>(scope:Scope,path:string,body?:unknown):Promise<T>{
    ...
    const response=await this.fetcher(new URL(path,this.base),{method:body===undefined?'GET':'POST',redirect:'error',signal:AbortSignal.timeout(path.includes('/jobs/')?90000:30000),headers:{[this.authHeader]:this.authHeader==='Authorization'?`Bearer ${token}`:token,'Content-Type':'application/json','X-Agent-Contract':CONTRACT_VERSION},body:body===undefined?undefined:JSON.stringify(body)});
