...
//
// 关键: 注册(POST records)与登录(auth-with-password)是匿名接口, 绝不能携带
// 任何登录凭证 —— 浏览器里若残留一个已失效账号的旧会话(如账号被后台删除),
// 这里附上去的旧 token 会被平台网关当成非法令牌在最外层挡回(TOKEN_MISSION),
// 表现为注册/登录一律"操作失败", 且请求根本到不了后端。匿名接口一律发干净请求。
function isAnonymousAuthRequest(reqUrl: string, method: string): boolean {
  const u = String(reqUrl || "")
  const m = String(method || "GET").toUpperCase()
  if (u.includes("/collections/users/auth-with-password")) return true
  if (m === "POST" && /\/collections\/users\/records\/?$/.test(u.split("?")[0])) return true
  return false
}

pb.beforeSend = (url, options) => {
  const method = (options.method || "GET") as string
  if (isAnonymousAuthRequest(typeof url === "string" ? url : (url as URL)?.href, method)) {
    const headers: Record<string, string> = { ...((options.headers as Record<string, string>) || {}) }
    delete headers["Authorization"]
    delete headers[PB_USER_AUTH_HEADER]
    delete headers["X-Vibex-Scoped-Token"]
